...
```
/subagents           # table of last 20 delegations — current session only
/subagents 50        # table of last 50 (cap at 100) — current session only
/subagents all       # table of last 20 delegations across all sessions
/subagents all 50    # table of last 50 across all sessions
/subagents stats     # per-type aggregates for this session
```

The default view is scoped to the **current session** — no cross-session noise. Use `all` to see the global history.

Example table output (v0.5.0):

```
| #   | When        | Type             | Description (≤40)                      | Status  | Duration | Input  | CacheR | CacheW | Output |
|-----|-------------|------------------|------------------------------------------|---------|----------|--------|--------|--------|--------|
| 1   | 47m ago     | general-purpose  | Always-on failed + session elapsed       | done    | 7m 32s   | 1      | 55.5k  | 807    | 608    |
| 2   | 1h ago      | sdd-spec         | Write spec for auth                      | done    | 1m 12s   | 3200   | —      | —      | 8400   |
```

**Token columns** (v0.5.0 — 4 separate columns replace the old `input/output` pair):
- **Input** — raw input tokens charged (usually tiny when cache is warm)
- **CacheR** — tokens served from prompt cache (read; these are cheap)
- **CacheW** — tokens written into the prompt cache (creation; one-time cost)
- **Output** — tokens generated by the model

Values ≥ 10 000 are shown with a `k` suffix (e.g. `55.5k`).
